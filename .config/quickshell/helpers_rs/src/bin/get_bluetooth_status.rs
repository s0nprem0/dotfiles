...
fn is_bluetooth_enabled() -> bool {
  ...
  out.lines().any(|line| line.contains("Powered:") && line.contains("yes"))
}
...
fn parse_upower_batteries() -> HashMap<String, f64> {
  ...
  for line in out.lines() {
    ...
    if let Some(s) = t.strip_prefix("serial:") {
      ...
      if let Ok(pct) = t.trim_start_matches("percentage:").trim().trim_end_matches('%').parse::<f64>() {
...
fn main() {
      ...
      for line in out.lines() {
        if let Some(rest) = line.strip_prefix("Device ") {
          if let Some(space) = rest.find(' ') {
            ...
            for l in info.lines() {
              let t = l.trim();
              if t.starts_with("Connected:") { connected = t.contains("yes"); }
              if t.starts_with("Paired:") { paired = t.contains("yes"); }
            }

            let battery = get_battery_for_device(&addr, &upower);

            devices.push(BluetoothDevice { name, address: addr, connected, paired, battery });
          }
        }
      }
    }
  ...
  devices.sort_by(|a, b| {
    if a.connected != b.connected { b.connected.cmp(&a.connected) }
