...
Item {
    ...
    ParallelAnimation {
        id: introAnim
        onStopped: { if (!root.closing) root.introCompleted() }
        NumberAnimation {
            target: root; property: "animSlide"
            from: root.slideFrom; to: root.slideTo
            duration: root.introDuration; easing.type: Easing.OutCubic
        }
        NumberAnimation {
            target: root; property: "animOpacity"
            from: 0; to: 1
            duration: root.introDuration; easing.type: Easing.OutCubic
        }
    }
    ...
    ParallelAnimation {
        ...
        NumberAnimation {
            target: root; property: "animSlide"
            from: root.slideTo; to: root.slideFrom
            duration: root.exitDuration; easing.type: Easing.InCubic
        }
        NumberAnimation {
            target: root; property: "animOpacity"
            from: 1; to: 0
