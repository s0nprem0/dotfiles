...
fn parse_sinks(
    sinks_json: &serde_json::Value,
    default_name: &str,
) -> (Vec<SinkInfo>, Option<SinkInfo>) {
    ...
    if let Some(arr) = sinks_json.as_array() {
        for s in arr {
            ...
            let is_bt =
                name.to_lowercase().contains("bluez") || name.to_lowercase().contains("bluetooth");
            ...
            if name == default_name {
                default_sink = Some(SinkInfo {
                    ...
                    volume: sink.volume,
                    muted: sink.muted,
                    is_bluetooth: sink.is_bluetooth,
                    sample_rate: sink.sample_rate.clone(),
                    icon: sink.icon.clone(),
                });
            }
            sinks.push(sink);
        }
...
fn parse_sources(
    ...
    default_name: &str,
) -> (Vec<SourceInfo>, Option<SourceInfo>) {
    ...
    if let Some(arr) = sources_json.as_array() {
        for s in arr {
            ...
            if name == default_name {
                default_source = Some(SourceInfo {
                    index: source.index,
...
fn get_diagnostics() -> Diagnostics {
    ...
    let sample_rate = Command::new("sh")
        .args([
            "-c",
            r#"pactl list sinks | grep "Sample Specification" | head -1 | awk '{print $NF}'"#,
        ])
        .output()
        .ok()
        .and_then(|o| {
...
fn get_media_info() -> (Vec<MediaSource>, Option<String>, Option<MediaInfo>) {
    let stored_player =
        std::fs::read_to_string(primo::cache_dir().join("current_media_player"))
            .ok()
    ...
    let active_player = stored_player
        .as_ref()
    ...
    let media = active_player.as_ref().and_then(|player| {
        ...
        let player_name =
            playerctl_metadata(player, "{{playerName}}").unwrap_or_else(|| player.clone());
        let title = playerctl_metadata(player, "{{title}}").unwrap_or_default();
        let artist = playerctl_metadata(player, "{{artist}}").unwrap_or_default();
...
fn main() {
    ...
    let vol = default_sink
        .as_ref()
        .map(|s| s.volume)
        .or_else(|| sinks.first().map(|s| s.volume))
        .unwrap_or(0);
    let muted = default_sink
        .as_ref()
        .map(|s| s.muted)
        .or_else(|| sinks.first().map(|s| s.muted))
        .unwrap_or(false);
