...
fn main() {
    ...
    if json_mode {
        let ports: Vec<JsonPortEntry> = entries
            .into_iter()
            .map(|e| JsonPortEntry {
                protocol: e.protocol,
                port: e.port,
...
fn parse_ss() -> Vec<PortEntry> {
    ...
    for line in output.lines().skip(1) {
        ...
        let proto = parts[0];
        let local = parts[4];
        let peer_str = if parts.len() > 5 { parts[5] } else { "*:*" };
        ...
        entries.push(PortEntry {
            label: format!("{:<6} {:<20} {:<5} {}", port, pname, proto, ip),
            pid,
            protocol: proto.to_uppercase(),
            port: port.to_string(),
            process: pname,
            address: ip.to_string(),
            peer: peer_str.to_string(),
        });
    }
